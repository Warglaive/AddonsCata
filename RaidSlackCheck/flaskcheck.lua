...
for ii = 1,GetNumRaidMembers() do local name, _, subgroup, _, _, _, _, online, isDead = GetRaidRosterInfo(ii)
	...
	for yy=1,#rscwillcheckthem do
		...
		for uu=1,#rscfoodtable do
			if UnitAura(rscwillcheckthem[yy],GetSpellInfo(rscfoodtable[uu])) then
				local _, _, _, _, _, _, expirationTime = UnitAura(rscwillcheckthem[yy],GetSpellInfo(rscfoodtable[uu]))
	...
	for yy=1,#rscwillcheckthem do
		...
		for uu=1,#rscflasktable do
			if UnitAura(rscwillcheckthem[yy],GetSpellInfo(rscflasktable[uu])) then
				local _, _, _, _, _, _, expirationTime = UnitAura(rscwillcheckthem[yy],GetSpellInfo(rscflasktable[uu]))
				if expirationTime==nil or (expirationTime and expirationTime-GetTime()>rscminflaskgood*60) or expirationTime==0 then
					byl=100
					uu=100
				end
		...
		for uu2=1,#rscflasktableold do
			if UnitAura(rscwillcheckthem[yy],GetSpellInfo(rscflasktableold[uu2])) then
				local _, _, _, _, _, _, expirationTime = UnitAura(rscwillcheckthem[yy],GetSpellInfo(rscflasktableold[uu2]))
				if expirationTime==nil or (expirationTime and expirationTime-GetTime()>rscminflaskgood*60) or expirationTime==0 then
					old=100
					byl=100
				end
	...
	for yy=1,#rscnoflask do
		...
		for uu=1,#rscelixirtable1 do
			if UnitAura(rscnoflask[yy],GetSpellInfo(rscelixirtable1[uu])) then
				local _, _, _, _, _, _, expirationTime = UnitAura(rscnoflask[yy],GetSpellInfo(rscelixirtable1[uu]))
				if expirationTime==nil or (expirationTime and expirationTime-GetTime()>rscminflaskgood*60) or expirationTime==0 then
					byl=1
					uu=100
				end
	...
	for yy=1,#rscnoflask do
		...
		for uu=1,#rscelixirtable2 do
			if UnitAura(rscnoflask[yy],GetSpellInfo(rscelixirtable2[uu])) then
				local _, _, _, _, _, _, expirationTime = UnitAura(rscnoflask[yy],GetSpellInfo(rscelixirtable2[uu]))
				if expirationTime==nil or (expirationTime and expirationTime-GetTime()>rscminflaskgood*60) or expirationTime==0 then
					byl=1
					uu=100
				end
...
if manual=="manual" or (manual=="auto" and (IsRaidOfficer()==1 or rscflaskcheckb[5]==1 or psfnopromrep) and (GetTime()>rscflaskdelayrep[2] or (GetTime()>rscflaskdelayrep[2]-37 and GetTime()<rscwasyellpull+4))) then
if manual=="auto" then
...
--whisper send
if rscflaskcheckb[4]==1 and GetTime()-rscflaskimportchat2[1]>100 and #whisper1>0 and manual=="auto" and (IsRaidOfficer()==1 or rscflaskcheckb[5]==1 or psfnopromrep) and (GetTime()>rscflaskdelayrep[1] or (GetTime()>rscflaskdelayrep[1]-37 and GetTime()<rscwasyellpull+4)) then
...
SetMapToCurrentZone()
if #rscignorezone3[1]>0 then
	for ug=1,#rscignorezone3[1] do
		if rscignorezone3[1][ug]==GetCurrentMapAreaID() then
			rscignorezone3[2][ug]=GetRealZoneText()
			mojn=0
		end
	end
end
...
if (GetTime()>rscflaskdelayrep[2] or (GetTime()>rscflaskdelayrep[2]-37 and GetTime()<rscwasyellpull+4)) and (IsRaidOfficer()==1 or rscflaskcheckb[5]==1 or psfnopromrep) then
...
if (GetTime()>rscflaskdelayrep[1] or (GetTime()>rscflaskdelayrep[1]-37 and GetTime()<rscwasyellpull+4)) and (IsRaidOfficer()==1 or rscflaskcheckb[5]==1 or psfnopromrep) and rscflaskcheckb[4]==1 then
...
if nn==1 then
	local rrr=0
	if #rscignorezone3[1]>0 then
		for i=1,#rscignorezone3[1] do
			if rscignorezone3[1][i]==GetCurrentMapAreaID() then
				rscignorezone3[2][i]=GetRealZoneText()
				rrr=1
			end
		end
	end
	if rrr==0 then
		table.insert(rscignorezone3[2],GetRealZoneText())
		table.insert(rscignorezone3[1],GetCurrentMapAreaID())
	...
	if #rscignorezone3[1]>0 then
		for i=1,#rscignorezone3[1] do
			if rscignorezone3[1][i]==GetCurrentMapAreaID() then
				...
				table.remove(rscignorezone3[1],i)
				table.remove(rscignorezone3[2],i)
				i=1000
			end
		end
	end
...
if nn==3 then
	local txt=""
	if #rscignorezone3[2]>0 then
		for i=1,#rscignorezone3[2] do
			txt=txt.."|cff00ff00"..rscignorezone3[2][i].."|r"
			if #rscignorezone3[2]>i then
				txt=txt..", "
			end
		end
	end
...
table.wipe(rscignorezone3[1])
table.wipe(rscignorezone3[2])
else
--тыкни еще раз!
